/**
 * SAFE CLEANUP SCRIPT FOR PROFORMA INVOICES
 * 
 * This script will:
 * 1. List all proforma invoices from the database
 * 2. Delete PDF files from AWS S3 (from the ProformaInvoice folder)
 * 3. Delete all proforma invoice records from MongoDB
 * 
 * IMPORTANT: This is a DESTRUCTIVE operation. Use with caution!
 * 
 * Usage:
 *   node backend/scripts/cleanupProformaInvoices.js
 * 
...
const cleanupProformaInvoices = async () => {
  try {
    // Connect to MongoDB
    ...
    console.log(`\n📊 Found ${totalCount} proforma invoice(s) in database`);
    
    if (totalCount === 0) {
      console.log('✅ No proforma invoices to clean up. Exiting.');
    ...
    console.log(`   - ${totalCount} proforma invoice record(s) from MongoDB`);
    ...
    const deleteResult = await ProformaInvoice.deleteMany({});
...
const isMainModule = process.argv[1] && process.argv[1].endsWith('cleanupProformaInvoices.js');

if (isMainModule) {
  cleanupProformaInvoices()
    .then(() => {
...
export { cleanupProformaInvoices };
