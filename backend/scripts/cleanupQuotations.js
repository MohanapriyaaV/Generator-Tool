/**
 * SAFE CLEANUP SCRIPT FOR QUOTATIONS
 * 
 * This script will:
 * 1. List all quotations from the database
 * 2. Delete PDF files from AWS S3 (from the Quotation folder)
 * 3. Delete all quotation records from MongoDB
 * 
 * IMPORTANT: This is a DESTRUCTIVE operation. Use with caution!
 * 
 * Usage:
 *   node backend/scripts/cleanupQuotations.js
 * 
...
const cleanupQuotations = async () => {
  try {
    ...
    console.log(`\n📊 Found ${totalCount} quotation(s) in database`);
    
    if (totalCount === 0) {
      console.log('✅ No quotations to clean up. Exiting.');
      await mongoose.disconnect();
      return;
    }
    
    // Show summary
    console.log('\n📝 Summary of quotations to be deleted:');
    quotations.forEach((quot, index) => {
      console.log(`  ${index + 1}. Quotation #${quot.quotationNo} (ID: ${quot._id})`);
      if (quot.s3Url) {
        console.log(`     S3 URL: ${quot.s3Url.substring(0, 80)}...`);
      }
    });
    
    // Ask for confirmation
    console.log('\n⚠️  WARNING: This will permanently delete:');
    console.log(`   - ${totalCount} quotation record(s) from MongoDB`);
    ...
    if (s3) {
      ...
      for (const quotation of quotations) {
        if (quotation.s3Url) {
          const s3Key = extractKeyFromUrl(quotation.s3Url);
          if (s3Key) {
            ...
            console.warn(`⚠️  Could not extract S3 key from URL: ${quotation.s3Url}`);
            s3SkippedCount++;
          }
        } else {
          s3SkippedCount++;
...
const isMainModule = process.argv[1] && process.argv[1].endsWith('cleanupQuotations.js');
