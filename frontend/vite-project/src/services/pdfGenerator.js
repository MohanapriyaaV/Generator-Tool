...
        ],
        ...
        // Tax Rows - only show if tax is enabled
        ...(() => {
          const taxEnabled = data.taxEnabled !== false; // Default to true if not provided
        ...
        pdfDoc.getBlob((blob) => {
          ...
          (async () => {
            try {
              ...
              // Update database with S3 URL and full invoice data if editing
              try {
                const { updateProformaInvoice, getAllProformaInvoices } = await import('./api.js');
                
                // Check if this is an edit (has _id in data)
                if (data._id) {
                  ...
                  // Just update S3 URL if no _id (shouldn't happen, but fallback)
